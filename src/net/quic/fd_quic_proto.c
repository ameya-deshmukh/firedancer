...
ulong
fd_quic_handle_initial_pkt( fd_quic_t * quic, uchar const * buf, ulong buf_sz ) {
  // TODO remove these:
  (void)quic;
  (void)buf;
  (void)buf_sz;
  ...
  ulong rc = fd_quic_decode_initial( initial_pkt, buf, buf_sz );
...
ulong
fd_quic_handle_pkt( fd_quic_t * quic, uchar const * buf, ulong buf_sz ) {
  ...
  ulong skip = fd_quic_decode_common_hdr( common_hdr, buf, buf_sz );
