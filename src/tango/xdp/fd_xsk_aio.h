...
/* fd_xsk_aio_{align,footprint} return the required alignment and
   ...
   batch_cnt is the max number of buffers to handle at per
   fd_xsk_aio_housekeep() operation. */
...
/* fd_xsk_aio_new formats an unused memory region for use as an
   ...
   fd_xsk_aio_align() and fd_xsk_aio_footprint().  batch_cnt is the
   number of batches this fd_xsk_aio_t instance can handle per
   fd_xsk_aio_housekeep() operation.  Returns handle suitable for
...
/* fd_xsk_aio_join joins the caller to the fd_xsk_aio_t.  xsk_aio points
   to the first byte of the memory region backing the fd_xsk_aio_t in
   the caller's address space.  Wraps xsk for use in aio interfaces.
   xsk must be a locally joined fd_xsk_t instance with a lifetime at
   least that of this fd_xsk_aio_t.  Returns a pointer in the local address
   space to the fd_xsk_aio_t on success or NULL on failure (logs
   ...
   fd_xsk_aio_t at any given time. */
...
/* fd_xsk_aio_set_rx sets the fd_aio_t instance called back when
   fd_xsk_t receives data.  Requires periodic fd_xsk_aio_housekeep()
...
/* fd_xsk_aio_get_tx gets the fd_aio_t instance to send data out to the
   network via the underlying fd_xsk_t.  Dispatches frames immediately
   to AF_XDP but requires periodic fd_xsk_aio_service() calls to poll
   for completion. */

fd_aio_t *
fd_xsk_aio_get_tx( fd_xsk_aio_t * xsk_aio );

/* fd_xsk_aio_housekeep services aio callbacks for incoming packets and
